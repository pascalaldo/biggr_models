...
def get_application(debug=False):
    routes = [
        ...
        (
            ...
            UniversalReactionListHandler,
        ),
        (r"/(?:models/)?universal/reactions/?$", UniversalReactionListDisplayHandler),
        #
        (
            r"/(?:api/%s/)?(?:models/)?universal/reactions/([^/]+)/?$" % api_v,
            UniversalReactionHandler,
        ),
        #
        (
            r"/api/%s/(?:models/)?universal/metabolites/?$" % api_v,
            UniversalMetaboliteListHandler,
        ),
        (
            r"/(?:models/)?universal/metabolites/?$",
            UniversalMetaboliteListDisplayHandler,
        ),
        #
        (
            r"/(?:api/%s/)?(?:models/)?universal/metabolites/([^/]+)/?$" % api_v,
            UniversalMetaboliteHandler,
        ),
        #
        ...
        (
            r"/api/%s/search_reaction_with_stoichiometry$" % api_v,
            ReactionWithStoichHandler,
        ),
...
class ReactionListHandler(PageableHandler):
    def get(self, model_bigg_id):
        ...
        raw_results = safe_query(queries.get_model_reactions, model_bigg_id, **kwargs)
        # add the URL
        if "include_link_urls" in self.request.query_arguments:
            raw_results = [
                dict(
...
class MetaboliteListHandler(PageableHandler):
    def get(self, model_bigg_id):
        ...
        # run the queries
        raw_results = safe_query(queries.get_model_metabolites, model_bigg_id, **kwargs)
        # add the URL
        if "include_link_urls" in self.request.query_arguments:
            raw_results = [
                dict(
...
class GeneListHandler(PageableHandler):
    def get(self, model_bigg_id):
        ...
        raw_results = safe_query(queries.get_model_genes, model_bigg_id, **kwargs)
...
class GeneHandler(BaseHandler):
    ...
    def get(self, model_bigg_id, gene_bigg_id):
        result = safe_query(queries.get_model_gene, gene_bigg_id, model_bigg_id)
        self.return_result(result)
